    ...
    <footer className="relative overflow-hidden border-t border-white/10 bg-[#030c1a] pb-24 md:pb-0">
      ...
      <div className="relative mx-auto max-w-[1400px] px-5 pt-20 pb-8 md:px-8">
        ...
        <div className="grid gap-10 py-12 md:grid-cols-3">
          ...
          <div><p className="mb-4 text-[11px] tracking-[0.2em] text-white/35 uppercase">Bizi takip edin</p><div className="flex items-center gap-4"><a href="https://instagram.com/sitenkolaydigital" target="_blank" rel="noreferrer" aria-label="Instagram · sitenkolaydigital" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 text-white/60 hover:border-blue hover:text-blue"><svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8"><rect x="3" y="3" width="18" height="18" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r=".8" fill="currentColor" stroke="none"/></svg></a><span className="flex items-center gap-2 text-sm text-white/45"><MapPin className="h-4 w-4 text-blue" />İstanbul, Türkiye</span></div></div>
        </div>
        <div className="flex flex-col justify-between gap-2 border-t border-white/10 pt-7 text-xs text-white/30 sm:flex-row"><p>© {new Date().getFullYear()} sitenkolay. Tüm hakları saklıdır.</p><p>Bir Luncrea kuruluşudur.</p><p>Sade · Modern · Etkili</p></div>
      </div>
    </footer>
  );
}
